...
mod tests {
    ...
    async fn test_get_token_owner() {
        ...
        mock_client.expect_call_contract().returning(|_, _, _, _| Ok(vec![FieldElement::from_dec_str("1").unwrap()]));

        let token_manager = TokenManager::new(&mock_storage, &mock_client);

        let result = token_manager.get_token_owner(contract_address, token_id_low, token_id_high).await;
